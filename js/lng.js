const lngArr = {
   'nav-home': {
      'ua': 'Головна',
      'eng': 'Home',
   },
   'nav-about': {
      'ua': 'Про себе',
      'eng': 'About',
   },
   'nav-skills': {
      'ua': 'Навички',
      'eng': 'Skill',
   },
   'nav-gallery': {
      'ua': 'Галерея',
      'eng': 'Gallery',
   },
   'nav-contacts': {
      'ua': 'Контакти',
      'eng': 'Contacts',
   },
   'hero-title': {
      'ua': document.innerHTML = 'Роман <span>Бузуков</span>',
      'eng': document.innerHTML = 'Roman <span>Buzukov</span>',
   },
   'hero-text': {
      'ua': document.innerHTML = 'Front-end розробник <span>36 років, м. Черкаси</span>',
      'eng': document.innerHTML = 'Front-end developer <span>36 years old, Cherkasy</span>',
   },
   'about-title': {
      'ua': 'Про себе',
      'eng': 'About me',
   },
   'about-text-1': {
      'ua': "Привіт, мене звати Роман - я Front-end розробник з м.Черкаси. Цікавлюсь web-дизайном, розробкою програмного забеспечення і всім, що пов'язано із цим.",
      'eng': document.innerHTML = "Hi, I'm Roman – Front-end developer from Cherkasy. <span>I'm interested in web-design, coding and everything connected with it.</span>",
   },
   'about-text-2': {
      'ua': 'Я навчаюсь в IT Академії по курсу "Front-end"',
      'eng': document.innerHTML = "I'm studying at <span>\"Front-end\"</span> courses in IT-Academy.",
   },
   'about-text-3': {
      'ua': 'Готовий втілювати чудові проєки з неймовірними людьми',
      'eng': 'Ready to implement excellent projects with wonderful people.',
   },
   'skills-title': {
      'ua': 'Навички',
      'eng': 'Skills',
   },
   'skills-text': {
      'ua': 'Мій рівень навичок в таких технологіях',
      'eng': 'My skills in such technologys as',
   },
   'skills-html': {
      'ua': 'Мова Розмітки Гіпертексту',
      'eng': 'HyperText Markup Language',
   },
   'skills-css': {
      'ua': 'Каскадні Таблиці Стилів',
      'eng': 'Cascading Style Sheets',
   },
   'gallery-title': {
      'ua': 'Галерея',
      'eng': 'Gallery',
   },
   'footer-title': {
      'ua': 'Контакти',
      'eng': 'Contacts',
   },
   'footer-text-1': {
      'ua': 'Хочете дізнатись про мене більше чи просто поспілкуватись?',
      'eng': 'Want to know more or just chat?',
   },
   'footer-text-2': {
      'ua': 'Буду дуже радий!',
      'eng': 'You are welcome!',
   },
   'footer-btn': {
      'ua': 'Надіслати повідомлення',
      'eng': 'Send message',
   },
   'footer-text-3': {
      'ua': 'Відвідайте мої профілі на сайтах',
      'eng': 'Like me on',
   },
}